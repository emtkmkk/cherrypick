...
<div ref="rootEl" :class="[$style.root, { [$style.reduceAnimation]: !prefer.s.animation, [$style.showEl]: (showEl && ['hideFloatBtnNavBar', 'hide'].includes(<string>prefer.s.displayHeaderNavBarWhenScroll)) }]">
	...
	<button v-if="store.s.showReloadButtonInNavbar" v-vibrate="prefer.s['vibrate.on.system'] ? 5 : []" :class="$style.item" class="_button" @click="reload()">
		<div :class="$style.itemInner">
			<i :class="$style.itemIcon" class="ti ti-refresh"></i>
		</div>
	</button>

	<button v-if="store.s.showPostButtonInNavbar" v-vibrate="prefer.s['vibrate.on.system'] ? 5 : []" :class="[$style.item, $style.post]" class="_button" @click="os.post()">
		<div :class="$style.itemInner">
